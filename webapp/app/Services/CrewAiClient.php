        ...
        try {
            ...
            $response = $request->post($baseUrl.$endpoint, [
                ...
                'instructions' => [
                    ...
                    'geopolitical_tension_region_rule' => 'Derive region_name and display_region_name dynamically from the title, summary and source content, without fixed lookup strings. Check the headline first, then the lead and body. region_name must be the broad country, state, macro-region, or geopolitical theater explicitly supported by the source, never a city, district, neighborhood, or admin1 label. display_region_name may be a city or province only when that place is explicitly mentioned in the source and clearly belongs to the story; if no city is explicit, keep display_region_name equal to region_name or leave it empty. Do not invent toponyms. If multiple places appear, prefer the main geopolitical theater described by the headline and opening paragraphs. Avoid generic labels like "Area non specificata" unless the source truly lacks geographic evidence.',
